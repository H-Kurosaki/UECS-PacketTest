...
U_orgAttribute.mac[1] = 0x14;
U_orgAttribute.mac[2] = 0x25;
U_orgAttribute.mac[3] = 0x25;
U_orgAttribute.mac[4] = 0x36;
U_orgAttribute.mac[5] = 0x36;
...
static signed long lastVal=-1;
  if(U_ccmList[CCMID_test1].validity)
      {
      //digitalWrite(DOUT_LED_LOW,HIGH);
      if(U_ccmList[CCMID_test1].value-lastVal==1)
        ...
        recvpacket_++;
        digitalWrite(DOUT_LED_HIGH,HIGH);
        lastVal=U_ccmList[CCMID_test1].value;
        }
      }
     else
     {
      //digitalWrite(DOUT_LED_LOW,LOW);
     }
...
pinMode(DOUT_LED_HIGH, OUTPUT);//LED
digitalWrite(DOUT_LED_HIGH,LOW);
UECSsetup();
}
